## 1. Checklists

<center>

### L01 - Acessar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L02 - Assinatura
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L03 - Assistir
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L04 - Conta
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖   |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L05 - Controlar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L06 - Criar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L07 - Destaque
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | Não |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✖   |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L08 - Dispositivo
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L09 - Documentário
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖   |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L10 - Download
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L11 - Duração
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L12 - Editar Perfil
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L13 - Episódio
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L14 - Excluir
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L15 - Explorar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L16 - Extra
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✖ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L17 - Faixa Etária
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L18 - Filtrar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L19 - Filme
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L20 - Gênero
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L21 - Grupo watch
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L22 - Idioma
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L23 - Legenda
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L24 - Lista
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L25 - Login
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L26 - Logout
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L27 - Novidades
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L28 - Original
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L29 - Pagamento
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L30 - Perfil
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L31 - Perfil Infantil
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L32 - Pesquisar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L33 - Pin
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L34 - Recomendado
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✖ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L35 - Renovar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L36 - Reprodução automática
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L37 - Responsividade
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L38 - Retomar
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L39 - Sair
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L40 - Segundo plano
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L41 - Selecionar perfil
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L42 - Senha
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L43 - Série
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L44 - Sinopse
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L45 - Streaming
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L46 - Temporada
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L47 - Tendência
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✖ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L48 - Trailer
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✖ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L49 - Título
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L50 - Usuário
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |

### L51 - Visualizar datalhes
| Número | Questão                                           | Resposta |
| :----: | :------:                                          | :------: |
| 1      | Possui todas as informações necessárias?          | ✔ |
| 2      | Há apenas informações referentes a esse artefato? | ✔ |
| 3      | As informações são concisas?                      | ✔ |
| 4      | Há apenas informações necessárias?                | ✔ |
| 5      | Uso de linguagem compreensível ao público?	     | ✔ |
| 6      | Possui classificação correta?                     | ✔ |
| 7      | Possui sinônimo correto?                          | ✔ |
| 8      | Possui impacto correto?                           | ✔ |
| 9      | Possui noção correta?                             | ✔ |
| 10     | Linguagem e ortografia estão corretas?            | ✔ |


</center>

## 2. Histórico de versão

| Versão | Data       | Descrição                                           | Autor        |
| ------ | ---------- | --------------------------------------------------- | ------------ |
| 0.1    | 17/09/2021 | Criação do documento de verificação de léxicos      | Juliana      |
| 0.2    | 21/09/2021 | Continuação da verificação dos léxicos      | Juliana      |

